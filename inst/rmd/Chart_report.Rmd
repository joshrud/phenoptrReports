...
params:
  ...
  colors:
    input: character
    label: RGBs for chart colors
    value: !r phenoptr_colors
  .by:
    input: select
...
```{r setup, echo=FALSE,include=FALSE,message=F}
...
scale_fill_phenoptr = scale_fill_manual(values=ifelse(colors=='', phenoptr_colors, colors))
...
``` {r test_colors,results='asis'}
p <- qplot(x=1:length(phenoptr_colors), y = 1, fill=factor(1:length(phenoptr_colors)), geom="tile") +
    scale_fill_manual(values = phenoptr_colors) +
    theme_void()+
